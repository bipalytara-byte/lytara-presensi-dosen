...
// [V10] Token WK I TIDAK lagi ditulis di sini. Dulu nilainya bisa dibaca
//       siapa saja lewat view-source. Sekarang token dikirim ke server
//       dan diverifikasi di sana (sheet Pengaturan → kunci "tokenWki").
async function tutupPaksa(id) {
  ...
  if (!inputToken) { alert('❌ Token tidak boleh kosong.'); return; }
  ...
  if (!jamSelesaiJadwal || jamSelesaiJadwal.indexOf(':') < 0) {
    ...
    var selisih = (now.getHours()*60 + now.getMinutes()) - ((+p2[0])*60 + (+p2[1]));
    if (selisih >= -5) {
  ...
  var infoPresensi = presensi
    ? '\n👤 ' + presensi.dosen + '\n📚 ' + presensi.mk
      ...
      + '\n🕐 Waktu tutup: ' + ws + '\n📋 Status: ' + stl
    : '';
  if (!confirm('Lanjutkan menutup paksa sesi ini?' + infoPresensi)) return;

  setSB('sy');
  try {
    var r = await post({
      action:'updateSelesai', id:id, waktuSelesai:ws,
      statusSelesai:stl, colorSelesai:stc, tokenWki:inputToken
    });
    if (!r || r.success !== true) {
      setSB('er');
      alert('❌ ' + ((r && r.error) || 'Gagal menutup sesi.'));
      return;
    }
